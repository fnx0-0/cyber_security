...
import { ChevronDown, ChevronRight, SquareChevronDown, SquareChevronRight } from 'lucide-react'
...
const mobileItems = [
    {
        ...
        "SOFTWARE_DESCRIPTION": "It refers to the process of storing, organizing, and tracking digital documents. It often involves the use of Document Management Systems (DMS), which provide tools to manage the lifecycle of documents, ensuring that they are securely stored, easily accessible, and can be shared or retrieved as needed.",
        "SOFTWARE_VERSION": 1,
...
function AppCards({ items }: { items: any[] }) {
    ...
    const { searchQuery, setSearchQuery, setCurrentSelectedAppData, filterData, setFilterData } = context;
    ...
    const areAllArraysEmpty = () => {
        return Object.values(filterData).every(arr => Array.isArray(arr) && arr.length === 0);
    };
    ...
    return (
        <>
            <div className={`${seeAllMobileApp ? 'hidden' : 'block'} flex flex-row gap-3 items-center my-3`}>
                {/* <h1 className='text-md font-bold'></h1> */}
                <Button size='sm' variant='outline' onClick={() => { setSeeAllWebApp((prev) => !prev) }}>
                    Web Application {seeAllWebApp ? <ChevronDown /> : <ChevronRight />}
                </Button>
            </div>
            <div className={`${seeAllWebApp ? 'h-[90%]' : 'h-[45%]'} ${seeAllMobileApp ? 'hidden' : 'block'} overflow-y-auto`}>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 '>
                    {items
                        ...
                        .map((item, index) =>
                            ["Base App", "SSD NEW", "Customer Support - Microservice", "IKON DevZone", "Water Management - TCS"].includes(item.SOFTWARE_NAME) || (index > 4 && !seeAllWebApp)
                                ?
                                null// <NoDataComponent /> 
                                :
                                (
                                    <Card key={item.SOFTWARE_ID}>
                                        <Link href={`/app-store/${item.SOFTWARE_ID}?name=${encodeURIComponent(item.SOFTWARE_NAME.replace(/\s+/g, "-"))}`}>
                                            <CardHeader>
                                                <AspectRatio ratio={16 / 9}>
                                                    <img src={imageMap[item.SOFTWARE_NAME] || "/assets/images/Slider-1-D.svg"} alt='App' className='rounded-md h-full w-full' />
                                                </AspectRatio>
                                            </CardHeader>
                                            <CardContent>
                                                ...
                                                <div className='truncate'>
                                                    <small title={item.SOFTWARE_DESCRIPTION}>{item.SOFTWARE_DESCRIPTION}</small>
                                                </div>
            ...
            <div className={`${seeAllWebApp ? 'hidden' : 'block'} flex flex-row gap-3 items-center my-3`}>
                {/* <h1 className='text-md font-bold'>Mobile Application</h1> */}
                <Button size='sm' variant='outline' onClick={() => { setSeeAllMobileApp((prev) => !prev) }}>
                    Mobile Application {seeAllMobileApp ? <ChevronDown /> : <ChevronRight />}
                </Button>
            </div>
            <div className={`${seeAllMobileApp ? 'h-[90%]' : 'h-[45%]'} ${seeAllWebApp ? 'hidden' : 'block'} overflow-y-auto`}>
                <div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 '>
                    {mobileItems
                        .filter((item) => {
                            return item.SOFTWARE_NAME.toLowerCase()?.includes(searchQuery);
                        ...
                        .map((item, index) =>
                            ["Base App", "SSD NEW", "Customer Support - Microservice", "IKON DevZone", "Water Management - TCS"].includes(item.SOFTWARE_NAME) || (index > 4 && !seeAllMobileApp)
                                ?
                                null
                                :
                                (
