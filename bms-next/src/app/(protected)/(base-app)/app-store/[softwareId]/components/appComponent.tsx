...
import { Check, ChevronRight, CircleCheck, Cog, Share2, Star } from 'lucide-react'
...
const appImagePath = "/assets/images/apps/";
...
export default function AppComponent({ params }: { params: { softwareId: string } }) {
    ...
    const [showMoreRequirements, setShowMoreRequirements] = useState<boolean>(true);
        ...
        <>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className='flex flex-col gap-3 pt-2'>
                            <p>
                                The official ChatGPT desktop app brings you the newest model improvements from OpenAI, including access to OpenAI o1-preview, our newest and smartest model.
                            </p>
                            <p>
                            ...
                            <div className='px-8'>
                                <ul className='list-disc'>
                                    <li>Instant answers—Use the [Alt + Space] keyboard shortcut for faster access to ChatGPT</li>
                                    <li>Chat with your computer—Use Advanced Voice to chat with your computer in real-time and get hands-free advice and answers while you work.</li>
                                    <li>Search the web—Get fast, timely answers with links to relevant web sources.</li>
                                    <li>Collaborate on writing and code—Use canvas to work with ChatGPT on projects that require editing and revisions.</li>
                                    <li>Stay productive—Keep ChatGPT easily accessible next to your open apps with the companion window.</li>
                                    <div className={`${showMoreDescription ? 'hidden' : 'block'}`}>
                                        <li>Take and share screenshots—Ask about anything on your screen</li>
                                        <li>Upload files and photos—Summarize and analyze multiple documents</li>
                                        <li>Get professional input—Brainstorm marketing copy or a business plan</li>
                                        <li>Creative inspiration—Get birthday gift ideas or create a personalized greeting card with DALL•E</li>
                                    </div>
                                </ul>
                            </div>

                            <p className={`${showMoreDescription ? 'hidden' : 'block'}`}>Join hundreds of millions of users and download ChatGPT today.</p>
                        </div>
                    </CardContent>
                    <CardFooter>
                        <Button variant='outline' className='mt-2' onClick={() => { setShowMoreDescription((prev) => !prev) }}>{showMoreDescription ? 'Show More' : 'Show Less'}</Button>
                    </CardFooter>
                </Card>
            </div>
                                ...
                                <div className="space-y-2 w-64">
                                    {ratings.map((rating) => (
                                        <div key={rating.stars} className="flex items-center gap-2">
                                            <span className={`flex items-center ${rating.stars === 1 ? 'gap-2' : 'gap-1'} text-sm font-medium`}>
                                                {rating.stars} <Star height={18} width={18} fill="gold" color="gold" />
                                            </span>
                                            <Progress value={rating.percent} className="w-40 h-2 bg-orange-100" />
                                        </div>
                                    ))}
                                </div>
                            </div>
                            <h2 className='font-semibold text-2xl my-3'>Review Summary Based on 38 Reviews</h2>
                            <p>Users rare about ChatGPT's sleek design, powerful search, and human-like Al. Despite minor performance issues, it's a must-have app for anyone who wants to keep up with technology.</p>
                            <h3 className='font-semibold text-lg mb-3'>Reviewers like this feature</h3>
                            <div className='flex flex-row gap-3'>
                                <Badge>Sleek</Badge>
                                <Badge>Powerful</Badge>
                                <Badge>Human Like</Badge>
                            </div>
                            <p className='mt-3 text-sm'>The above summary hase been generated by AI</p>

                        </div>

                    </CardContent>
                    <CardFooter>
                        <Button variant='outline' className='mt-2' onClick={() => { setOpenReview(true) }}>Show Review</Button>
                    </CardFooter>
                </Card>
            </div>
                    ...
                    <CardContent className='pt-1'>
                        {
                            features.map((feature, index) => (
                                index >= 3 ?
                                    <p key={index} className={`my-1 ${showMoreFeature ? 'hidden' : 'block'}`}>{feature}</p> :
                                    <p key={index} className='my-1'>{feature}</p>
                            ))
                        }
                        {showMoreFeature && <p>. . .</p>}
                    </CardContent>
                    <CardFooter>
                        <Button variant='outline' onClick={() => { setShowMoreFeature((prev) => !prev) }}>{showMoreFeature ? 'Show More' : 'Show Less'}</Button>
                    </CardFooter>
                </Card>
            </div>
            ...
            <div className='mt-3'>
                <Card>
                    ...
                    <CardContent>
                        <div className='flex flex-col gap-2'>
                            ...
                            <div className='flex flex-row gap-3 my-3'>
                                ...
                                <p>This product should work on your device. Items with a checkmark meet the developers system requirements.</p>
                            </div>
                        </div>
                        <div className={`flex flex-col gap-1 ${showMoreRequirements ? 'hidden' : 'block'} px-1`}>
                            {
                                systemRequirements.map((requirements, index) => (
                                    <div key={index} className='flex flex-row gap-3'>
                                        <Check height={15} width={15} color="green" className='self-center' />
                                        <p><span className='font-semibold'>{Object.keys(requirements)}</span> : {Object.values(requirements)}</p>
                                    </div>
                                ))
                            }
                        </div>
                    ...
                    <CardFooter>
                        <Button variant='outline' onClick={() => { setShowMoreRequirements((prev) => !prev) }}>{showMoreFeature ? 'Show More' : 'Show Less'}</Button>
                    </CardFooter>
                </Card>
            </div>
            ...
            <div className='mt-3'>
                <Card>
                    <CardHeader className='border-b'>
                        <CardTitle className='text-accent'>Additional Information</CardTitle>
                    </CardHeader>
                    <CardContent>
                                    ...
                                    <div key={index} className='flex flex-col'>
                                        ...
                                        <div className='flex flex-col'>
                                            {Object.values(additionalInfo).map((infoVal, index) => (
                                                <p key={index} className='text-lg'>{infoVal}</p>
                                            ))}
                                        </div>
                                    </div>
                                ))
