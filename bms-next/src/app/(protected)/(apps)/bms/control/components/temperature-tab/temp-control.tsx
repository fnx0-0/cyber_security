...
export default function TempControl() {
    // HVAC controls
    const { temperature, setTemperature, fanSpeed, setFanSpeed, hvacMode, setHvacMode, hvacPower, setHvacPower } = useHvac()
    const [loading, setLoading] = useState(true);
    // const [initialTemperature, setInitialTemperature] = useState(temperature); // Default temperature
    useEffect(() => {
        async function fetchInitialData() {
            ...
            if (data && data.length > 0) {
                ...
                // setInitialTemperature(temperature); // Set the initial temperature state
                setLoading(false);
            }
        }
        fetchInitialData()
    ...
    const handleReset = () => {
        ...
        // setTemperature(initialTemperature); // Reset the slider to the current temperature state
    };
