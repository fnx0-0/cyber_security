...
export default function RFPTamplate({
  ...
  const columns: DTColumnsProps<RfpTemplate>[] = [
    ...
    // },
    {
      ...
      cell: ({ row }) => (
        <Link href={"./my-templates/template-editor/" + row.original?.templateId}>
    ...
    },
    {
      ...
      header: () => (
        <div style={{ textAlign: "center" }}>Template Category</div>
      ),
      cell: ({ row }) => <span>{row.original?.templateCategory}</span>,
    },
    {
      id: "actions",
