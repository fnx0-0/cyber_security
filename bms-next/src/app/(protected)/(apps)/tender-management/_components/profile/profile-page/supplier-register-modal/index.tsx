...
interface SupplierRegistrationModalProps {
  ...
  openedFrom?: string | null | undefined;
...
export default function SupplierRegistrationModal({
    ...
    () => {
      const fetchDraftData = async () => {
        if (accountId && isOpen) {
          //setIsLoading(true);
          try {
            const formData: any = await getSupplierRegistrationData(accountId);
  ...
  const onSubmit = async (data: SupplierFormData) => {
    ...
    try {
      if (accountId) {
        ...
        await editSupplierRegistrationData(accountId, data);
  ...
  const onStartBid = async (data: SupplierFormData) => {
    console.log("Supplier Data start bid:", data);
    try {
      let draftDetails = null;
      if (tenderId && tenderId !== "") {
        draftDetails = await getProjectDetailsData(tenderId);
      }
      ...
      console.log(data);
      if (accountId) await editSupplierRegistrationData(accountId, data);
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-[80vw] h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Supplier Registration</DialogTitle>
        </DialogHeader>
          ...
          <FormProvider {...form}>
            ...
            >
              ...
              <div className="flex justify-between pt-4">
                ...
                {openedFrom === "review" && (
                  <Button type="button" onClick={form.handleSubmit(onStartBid)}>
                    Start Bidding
                  </Button>
                )}
