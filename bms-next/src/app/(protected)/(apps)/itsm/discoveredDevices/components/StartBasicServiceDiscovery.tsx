...
export default function StartBasicServiceDiscovery({open, close, deviceData}: StartBasicServiceDiscoveryProps){
    //console.log('devicedata inside StartBasicServiceDiscovery ', deviceData);
    const selectedDeviceIdArray = deviceData.map(obj=>obj.deviceId);
        ...
        if(instanceData.length){
            const invokeStartMonitoring = async function(i: number, len: number){
                if(i == len){
                    ...
                    // if (deviceWiseSelectedProbe != undefined && deviceWiseSelectedProbe[data.deviceId]) {
                    //     data.probeId = deviceWiseSelectedProbe[data.deviceId];
                    // }
