          ...
          return (
            <li key={currentIndex} className="lg:pl-2">
              <div
                //href={item.href || ''}
                className={
                  "flex items-center gap-1 p-2 justify-between hover:bg-accent hover:text-accent-foreground rounded-md cursor-pointer" +
...
export function AppSidebarProvider({ children }: { children: ReactNode }) {
  ...
  return (
    <AppSidebarContext.Provider
      value={{ menuItems, addMenuItems, clearMenuItem }}
    >
      {children}
    </AppSidebarContext.Provider>
  );
