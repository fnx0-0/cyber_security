...
export default function AccountSwitcher({ accounts, activeAccountDetails }: { accounts: AccountTreeProps[],activeAccountDetails: AccountTreeProps }) {
    const router = useRouter()
    const { isMobile } = useSidebar()
    //const [accounts, setAccounts] = useState<AccountTreeProps[]>([]);
    const [activeAccount, setActiveAccount] = useState<AccountTreeProps>(activeAccountDetails)
                ...
                >
                    <div className="flex aspect-square size-8 items-center justify-center rounded-lg">
                        <Avatar className="h-8 w-8 rounded-lg">
                            <AvatarImage src={activeAccount?.ACCOUNT_NAME === "Keross" ? "/assets/images/dark/symble-keross.png" : ""} alt={activeAccount?.ACCOUNT_NAME} />
                    ...
                    >
                        <div className="flex size-6 items-center justify-center rounded-sm border">
                            <Avatar className="h-6 w-6 rounded-lg">
                                <AvatarImage src={account?.ACCOUNT_NAME === "Keross" ? "/assets/images/dark/symble-keross.png" : ""} alt={account?.ACCOUNT_NAME} />
                                <AvatarFallback className="rounded-lg">{account?.ACCOUNT_NAME?.substring(0, 1)}</AvatarFallback>
                            </Avatar>
                        </div>
